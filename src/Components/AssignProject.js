...
const AssignProject = (params) => {
  ...
  console.log("Assign Project ===>", params);
  // Bootstrap Tooltip
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          let temp = [];
          res.data.map((val) => {
            if (params["params"]?.userID !== val.userID) {
              temp.push(
  ...
  return (
    <>
      <div style={{ margin: "10px" }}>
        ...
        <div className="row col-lg-12 borderdivstyle" style={{ margin: "0px" }}>
          <div className="row col-lg-4" style={{ padding: "0px" }}>
