...
const StackrowChart = ({ dataParentToChild }) => {
    ...
    React.useEffect(() => {
        ...
        let data = [
            { WorkflowStage: "Disclosure Signed", LoanPurpose: "Purchase", TotalVolume: 1000, EstimatedVolume: 800 }
            , { WorkflowStage: "Disclosure Signed", LoanPurpose: "Refinance", TotalVolume: 2000, EstimatedVolume: 1500 }
            , { WorkflowStage: "Appraisal Ordered", LoanPurpose: "Purchase", TotalVolume: 3000, EstimatedVolume: 2000 }
            , { WorkflowStage: "Appraisal Ordered", LoanPurpose: "Refinance", TotalVolume: 1500, EstimatedVolume: 700 }
            , { WorkflowStage: "Locked", LoanPurpose: "Purchase", TotalVolume: 1000, EstimatedVolume: 800 }
            , { WorkflowStage: "Locked", LoanPurpose: "Refinance", TotalVolume: 2200, EstimatedVolume: 1800 }
            , { WorkflowStage: "Registered", LoanPurpose: "Purchase", TotalVolume: 1300, EstimatedVolume: 500 }
            , { WorkflowStage: "Registered", LoanPurpose: "Refinance", TotalVolume: 1900, EstimatedVolume: 500 }
            , { WorkflowStage: "Suspend", LoanPurpose: "Purchase", TotalVolume: 1400, EstimatedVolume: 600 }
            , { WorkflowStage: "Suspend", LoanPurpose: "Refinance", TotalVolume: 2300, EstimatedVolume: 1800 }
            , { WorkflowStage: "Conditional Approval", LoanPurpose: "Purchase", TotalVolume: 1800, EstimatedVolume: 1500 }
            , { WorkflowStage: "Conditional Approval", LoanPurpose: "Refinance", TotalVolume: 3100, EstimatedVolume: 2100 }
            , { WorkflowStage: "Resubmission", LoanPurpose: "Purchase", TotalVolume: 4000, EstimatedVolume: 3500 }
            , { WorkflowStage: "Resubmission", LoanPurpose: "Refinance", TotalVolume: 3800, EstimatedVolume: 2800 }
            , { WorkflowStage: "Approved For Closing Docs", LoanPurpose: "Purchase", TotalVolume: 2800, EstimatedVolume: 1800 }
            , { WorkflowStage: "Approved For Closing Docs", LoanPurpose: "Refinance", TotalVolume: 3000, EstimatedVolume: 1900 }
            , { WorkflowStage: "Docs Sent To Title", LoanPurpose: "Purchase", TotalVolume: 2000, EstimatedVolume: 1000 }
            , { WorkflowStage: "Docs Sent To Title", LoanPurpose: "Refinance", TotalVolume: 1000, EstimatedVolume: 500 }
            , { WorkflowStage: "Docs Returned From Title", LoanPurpose: "Purchase", TotalVolume: 1000, EstimatedVolume: 800 }
            , { WorkflowStage: "Docs Returned From Title", LoanPurpose: "Refinance", TotalVolume: 1000, EstimatedVolume: 800 }
        ]
        //data = dataParentToChild[0];

        var range = [];
        var size = [];
        var Purchase = 0, Refinance = 0, Estimated_Purchase = 0, Estimated_Refinance = 0;
        for (var i = 0; i < data.length; i++) {
            if (range.indexOf(data[i].WorkflowStage === -1)) range.push(data[i].WorkflowStage);
        ...
        if (myTable != undefined) {
            ...
            myTable.rows[1].cells[3].innerHTML = formatter.format(Estimated_Refinance)
        }
        ...
        var grp1 = dimension.group().reduceSum(function (d) { return d.LoanPurpose === "Purchase" ? d.TotalVolume : 0 });
        var grp2 = dimension.group().reduceSum(function (d) { return d.LoanPurpose === "Refinance" ? d.TotalVolume : 0 });
        var grp3 = dimension.group().reduceSum(function (d) { return d.LoanPurpose === "Purchase" ? d.EstimatedVolume : 0 });
        var grp4 = dimension.group().reduceSum(function (d) { return d.LoanPurpose === "Refinance" ? d.EstimatedVolume : 0 });
        // var tipvalue = 'title';
     ...
     //       .useRightYAxis(true)
            .colors(d3.scaleOrdinal(['rgba(92, 179, 255)', 'rgba(92, 179, 255, 0.6)']))
            .group(grp1, "grp1 Bar").stack(grp2, "grp2 Bar");
        ...
        let barChart2 = dc.barChart(compositeChart)
         ...
         //   .useRightYAxis(true)
            .colors(d3.scaleOrdinal(['rgba(144, 238, 144)', 'rgba(144, 238, 144,0.6)']))
            .group(grp3, "grp3 Bar").stack(grp4, "grp4 Bar");
        ...
        compositeChart
            ...
            .renderlet(function (chart) {
                ...
                chart.selectAll("g.yr text")
                    .attr('x', '-25')
                    //.attr('text-anchor', 'end')
                    .attr('transform', "rotate(210)");

                chart.selectAll('rect').call(rowtip).on("click", function (d) {
                    d.stopPropagation();
                    tipremove();
                ...
                chart.selectAll('g text.barLabel')
    ...
    return (
        <Grid item xs={12} md={12} xl={6} lg={6} className="containerfit">
            <Grid item className="cardbox" style={{ height: "fit-content" }}>
                <div style={{ justifyContent: 'center', display: 'flex', fontWeight: 'bold' }}>Pipeline Analysis</div>
              ...
              <td>Value4</td>
                ...
                <div style={{ justifyContent: 'center', display: 'flex' }}>
                    <div className="legendhead">
                        <span className="legendsquare" style={{ backgroundColor: "#5CB3FF" }}></span>
                        <span>Purchase</span>
                    </div>
                    <div className="legendhead">
                        <span className="legendsquare" style={{ backgroundColor: "#5cb3ff99" }}></span>
                        <span>Refinance</span>
                    </div>
                    <div className="legendhead">
                        <span className="legendsquare" style={{ backgroundColor: "#90ee90" }}></span>
                        <span>Estimated Purchase</span>
                    </div>
                    <div className="legendhead">
                        <span className="legendsquare" style={{ backgroundColor: "#90ee9099" }}></span>
                        <span>Estimated Refinance</span>
                    </div>
                </div>

                <div className="boxcenter" id="divstackrowchart"  ref={div}>
                </div>
            </Grid>
        </Grid>
    );
